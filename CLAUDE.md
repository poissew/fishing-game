...
├── DotSpellData          — adds interval, duration, stack_multiplier; an ON_HIT effect left behind
├── SlamSpellData         — adds reach, min_drop, slam_scale; fires WHEN_OVER_TARGET
├── PaparazziSpellData    — adds spread, max_stun; fires WHEN_READY, deals no damage
...
├── WardSpellData         — adds resistance; a PASSIVE that turns a share of magic aside
└── LaserSpellData        — adds charge_time, beam_time, radius; a WHEN_READY beam
...
SpellBeam (Sprite3D)      — a bar of light laid along the shot; fades, frees itself
```
...
- `UIBattle` (`classes/ui/UI_Battle.gd` + `ui/UI_Battle.tscn`) is **a placeholder for the arena**, not the arena: it holds the BATTLE phase open, shows the champion, and closes the round out on Enter. **Building the arena means replacing it** — connect to `gamephase.battle_started(fish)`, run the fight with a `BattleRound` for the clock and the verdict (see **Battle rounds**), then call `gamephase.end_battle(won)`. Nothing else in the game reaches into that file.
...
- `BattleRound` (`classes/battle_round.gd`, Node) is the clock a fight runs on and the thing that decides who won it. Hand it the battlers with `start()`, listen for `finished(winner)`. It owns no scene and draws nothing, and it never touches `gamephase` — whoever runs the arena decides what winning means for the round after.
- It is **game code, not dev code**, even though `dev/TestArena.tscn` is the only thing using it today: a real arena needs a clock and a winner exactly as much as the test one does.
- A round is won outright when one fish kills the other — `BattleRound` watches `died` itself, so there is one place that knows who won however the round ended. Otherwise the clock runs out after `duration` (60 s) and **sudden death** starts: every fish still standing loses `drain_base + drain_growth * (n-1)` health every `drain_interval`, so the first tick costs 1, the fifth costs 5. Damage goes through `BattleFish.take_tick_damage()` — no source, no shove, since a fish knocked back by the passage of time would be a strange sight.
- **The growth is what guarantees an end.** A flat drain against an 86-health shark would take a minute and a half on its own; escalating, nothing survives long.
- **A tick can never kill the last two fish together.** The drain hits the weakest first and stops the moment one fish is left standing, so the survivor is spared that tick and there is always somebody to hand the round to. `finished(null)` exists for completeness and should not happen.
- Measured: two cowards, who cannot resolve a fight between them by any other means, on an 8-second clock — drains of 1, 2, 3, 4, 5 and the round decided at 12.9 s, the weaker fish dead and the other untouched by the last tick. An ordinary pair on the same clock never reached the drain and was decided by a kill at 7.0 s.
- The test arena draws the countdown centre-top (red under 10 seconds) and then a blinking `SUDDEN DEATH -N` with what the next tick costs.
...
- **Damage says what kind it is**, which is what decides which of a fish's defences gets a say. `BattleFish.DamageKind` has three: `PHYSICAL` (a touch, or a PHYSICAL spell — `ArmourSpellData` takes the edge off), `MAGICAL` (any other spell — `WardSpellData` turns a share aside) and `UNTYPED` (nothing defends against it). `take_damage()` is the touch path and is PHYSICAL by definition; `take_blast()`, `take_tick_damage()` and `slam_down()` take a `kind` that defaults to `UNTYPED`, and every spell passes `BattleFish.kind_of(data)`.
- **`UNTYPED` is load-bearing.** It covers the sudden-death drain and the blow Immunity was holding off finally landing. This was a plain `physical` bool until there was something that defended against magic, at which point `false` turned out to mean three different things at once — and a ward that applied to all three would have let a fish sit out the clock *and* survive its own last stand. If a new kind of defence turns up, it gets a `DamageKind`, not another bool.
...
- **`dev/TestArena.tscn`** (`dev/test_arena.gd` + `dev/arena_hud.gd` + `dev/free_camera.gd`) is where to watch a fight: two random species dropped in a walled box, HP bars and a hit log. It runs a `BattleRound` per pair (see **Battle rounds**), so a fight that cannot resolve itself still ends. **R** fresh pair, **1** rematch of the same two healed up, **E** the same rematch with both fish handed a spell they did not roll, stepping to the next one in `FishData.SPELL_POOL` on each press and finishing on the whole pool (one at a time because handing both fish everything includes Coward, and two cowards spend the round running away from each other), **Esc** hands the cursor back and then quits. Run it on its own with F6 — nothing in the game loads it and it never touches `gamephase`. It renders through the same 480x270 `SubViewport` as `main.tscn`, so it pixelates the way the real arena will.
...
- **`BattleFish.phys_dmg()` / `magic_dmg()` are the arena's stats, not the fish's.** Passives rearrange what a fish's numbers mean in a fight, and those two accessors are where it happens; the `FishInstance`'s own `phys_dmg` / `magic_dmg` are never touched, so the selection screen keeps reporting the catch honestly. `caster_power(data)` picks whichever of the two a spell scales off, and **every cast goes through it** — `SpellInstance.try_cast_at(power)` rather than `try_cast(fish)`, which still exists for simulating a fight with no bodies in it. That is what makes a passive reach a fish's spells and not just its touches.
...
- **`SpellData.Trigger` is append-only.** Godot writes an enum into a `.tres` as the plain int, so slipping a new trigger in among the existing ones silently re-points every resource after it. `WHEN_READY` first went in above `WHEN_OVER_TARGET` and turned Ground Slam into a spell waiting on a moment nothing ever handed it — it simply stopped firing, with no error anywhere.
- `SpellData.trigger` says what sets a spell off, and `FishInstance.ready_spells(trigger)` is the one lookup for all of them — every spell waiting on that trigger and off cooldown. **All** of them rather than the first: two spells that both go off on a touch have two separate cooldowns, and a fish carrying both gets both on the same touch. (A second AT_JUMP_PEAK spell is the exception — the first one freezes the fish, so the rest wait for the next hop.)
  - `MANUAL` waits to be cast by something else. Nothing uses it yet.
  - `ON_HIT` is armed the moment it comes off cooldown and is spent on the caster's **next contact of any kind** — another fish, a wall, the floor at the end of a flop. `BattleFish._cast_on_hit()` spends it, hung off `body_entered` rather than off `_hit()` so that a touch which is not allowed to do damage (a team mate, a fish already dead) still counts. A spell that needs something to *do something to* sets **`needs_hit`** and gets only touches that connected with an enemy — past the team check and the per-target cooldown — which `_hit()` reports back. Connecting is deliberately not the same as hurting: a coward connects and deals nothing, and a spell it carries that goes off on a hit still goes off. The cooldown is the only gate, which is what keeps a fish from going up every time it lands: over a 60-second armed soak in `TestArena` two fish on a 6-second cooldown cast 30 times between them, so in practice it fires the moment it comes back up.
  - `ON_LANDING` is spent the moment the fish puts something solid back under itself — the other end of the hop from `AT_JUMP_PEAK`, and spotted the same cheap way: `BattleFish._tick_landing()` watches `_grounded` go from false to true. `_was_grounded` is updated whatever happens next, so a fish that lands stunned does not fire the moment it comes round.
  - `WHEN_READY` goes off the instant it is off cooldown, with nothing to wait for and nothing to aim at. The cooldown is the whole of the pacing.
  - `WHEN_OVER_TARGET` is a **condition rather than a moment**: `BattleFish._tick_overhead()` looks every frame for a fish it may hit that is underneath this one, and hands the cast over whenever it finds one, so the spell's own cooldown is the only thing pacing it.
...
- **Coward** (`data/spells/coward.tres`, `CowardSpellData`) is the passive. A fish carrying it hops *away* from the nearest fish it could fight instead of towards it — `_flop_direction()` flips the heading and swaps `chase_bias` for `flee_bias` (0.9), keeping the same `_chase_strength()` falloff, so it barely reacts to something across the arena and bolts from something next to it. `_flee_from()` picks what it runs from: **a decoy if something has pointed it at one** (see Decoy below), and otherwise whoever is actually closest rather than whatever `chase_target` happens to hold — the thing about to catch it is the thing worth running from.
- **It will not fight in either direction.** `phys_scale` (0.0) takes the carrier's contact damage to nothing — `BattleFish.phys_dmg()` applies it, so the `FishInstance.phys_dmg` stat itself is untouched and the selection screen still reports the catch honestly. On open ground the fish never makes contact at all: measured, 1.6 → 31 units out over 25 seconds with its target untouched, against a control fish that stayed within 2.6 and dealt 35. In the arena the walls do the cornering, and then it stands there and takes it.
- **So a coward wins on its other spells or not at all**, and that is the whole shape of the spell. Three rounds of a bare coward against an ordinary fish: it landed **zero** damage in all three, died in two (6.5 s and 16.4 s) and in the third was still alive on 6 of 13 health when the 90-second cap ran out — a weak attacker may simply not be able to finish it. Give the same coward Bubble Blast and it won all three rounds on 28/31, 3/12 and 8/14 health, kiting and shooting. **Two** cowards in one arena will never finish a fight in either direction.
- **Trauma** (`data/spells/trauma.tres`, `DotSpellData`) is damage left behind rather than dealt. Landing a touch on an enemy puts an effect on **that fish** — `BattleFish.apply_dot()`, so the state lives on the victim, not on whoever caused it — worth `magic_dmg * damage_scale` (half) a tick, one tick a second for `duration` (4 s). It has **no cooldown**; the only thing rationing it is `HIT_COOLDOWN`, the same 0.45 s gate that stops two fish resting against each other trading damage every frame.
- **Reapplying multiplies, it does not refresh.** A touch landed on a fish already under the effect multiplies the running damage by `stack_multiplier` (2.0) and leaves the number of ticks alone, so the clock runs from the *first* touch whatever happens after. Measured with the touches applied by hand: ticks at 1, 2, 3 and 4 seconds, 5 damage each for a MAG 10 fish, a second touch at 2.5 s taking the last two ticks to 10 and the effect still ending at exactly 4.0 s. The doubling is unbounded on paper and shallow in practice — fish bounce apart between touches, so over four Trauma-against-Trauma rounds in `TestArena` a tick never got past about 4× its base and rounds ended in 4 to 10 seconds. If that ever stops being true, the cap belongs on the resource as a max-stacks field rather than in the ticking.
- Ticks go through `take_tick_damage()`, which is `take_damage()` with `NO_ORIGIN` and no shove — there is no direction for a tick of damage to have come from — and the fish that started it is credited through `report_indirect_damage()`, so a battle log reads the same whether the damage was a touch or something that touch left behind. One effect slot per fish, not one per attacker: a second application from *anyone* multiplies what is already there. The test arena's fighter cards show it as `DOT <ticks>x<damage>`, alongside `STUN <seconds>`.
- **Ground Slam** (`data/spells/ground_slam.tres`, `SlamSpellData`) waits for the carrier to be standing over somebody: a fish it may hit, within `reach` (1.0) to either side and at least `min_drop` (0.35) **below** it, is driven into the floor and loses the carrier's whole `magic_dmg`. `min_drop` is what makes it a slam and not a nudge — two fish side by side on the floor sit at nearly the same height, so without a drop to clear it would fire the moment they touched. The type is ARCANE for the same reason every other damaging spell is something other than PHYSICAL: that is what makes `SpellInstance.caster_power()` read `magic_dmg`.
- It goes through `BattleFish.slam_down()`, which is the one shove in the arena aimed **down** rather than away from something — `_shove()` takes a `lift` argument for it, 0 instead of the `KNOCKBACK_LIFT` that skips a fish back off the floor — and the damage carries `NO_ORIGIN` so nothing pushes the target sideways as well. A fish already on the floor just takes the damage; the impulse has nowhere to put it.
- **The condition is rarer than a touch but not rare.** Over 180 seconds and 35 rounds of Ground Slam against Ground Slam in `TestArena`, 56 slams landed, about 1.6 a round — fish get above each other through flops, knockback and blasts often enough. It will not show up in a short round: a 60-second arena soak cycling all five spells never once armed it during a window where the condition came up.
- **Paparazzi** (`data/spells/paparazzi.tres`, `PaparazziSpellData`) fires the moment it is off cooldown (8 s) whether or not anything is in front of it, and **deals no damage at all**. Anything the carrier may hit that is caught in the cone — `spread_degrees` (45) either side, out to `max_range` (2.5) — is stunned, for `max_stun` (5 s) with the lens against it down to `min_stun` (0.5 s) at the far edge. That falloff is the shotgun: the spread is the same at any range, what changes is how much of it lands.
- **A stunned fish does nothing whatsoever** — `is_stunned()` gates flopping, all four casting paths and its own `_hit()`, so it deals no contact damage to whatever walks into it. It can still be hurt, and a `DotSpellData` effect already on it keeps ticking. `stun()` takes the longest of the old and new durations rather than adding them, and interrupts a bubble volley. The sprite washes out towards `STUN_COLOR` while it lasts, because a fish standing still because it is stunned and a fish standing still because nothing is happening otherwise look identical.
- **The camera aims at the nearest fish in range**, not straight down the sprite's nose. A fish here is always drawn facing the camera and flipped rather than turned, so `facing()` is only ever the way it was last thrown — hardly ever at its opponent. Fired strictly down that line the spell barely existed: **50 flashes over 120 seconds of arena landed one stun**. Turning to face the subject is what a photographer does anyway, and the cone still decides who *else* is in the shot. With it: **34 flashes, 18 stuns**, the longest 4.69 s. Reverting to the strict reading is one line — `_flash_direction()` returning `facing()` unconditionally.
- Two fish both carrying it stun-lock each other and rounds drag: 21 rounds in 120 seconds became 8. That is what `BattleRound`'s sudden death is for.
- **Boxer** (`data/spells/boxer.tres`, `BoxerSpellData`) is the second passive, and a straight swap: `conversion` (1.0) of the fish's magic damage is added to its physical damage, and `magic_scale` (0.0) is what it has left. A MAG 9, PHYS 6 fish fights as PHYS 15, MAG 0 — and a Detonate in its fins is worth **0** instead of 14, because the conversion reaches the spells through `caster_power()`. It is a conversion, not a bonus.
- **Contact is worth more than a spell, and this proves it.** 28 rounds of a boxer against an ordinary fish carrying no spells: 21–7 to the boxer, which is only to be expected when the other fish's magic was doing nothing anyway. Against a fish carrying **Detonate**, so its magic is worth something: still 19–9. Turning a 6-second cooldown blast into permanently harder fists is a good trade at 1.0, and `conversion` is the dial if that is too good.
- **Sandbag** (`data/spells/sandbag.tres`, `ArmourSpellData`) is the third passive: every **physical** hit is worth `flat` (2 on the resource) plus `per_power` (0.5) of the fish's magic damage less than it was thrown, to a minimum of nothing. A MAG 9 fish shrugs off 7, so a 10-damage touch costs it 3 and a 5-damage one costs it nothing at all.
- **Armour comes off the damage, never off the shove.** A touch it absorbs entirely still knocks it about — measured, 0 health lost and a metre of travel — because `_knock_back()` is worked out from what the hit was thrown with, not from what got through. A sandbagged fish is harder to hurt, not harder to move; without that it would quietly become a wall.
- **A spell's damage goes straight past it** unless the spell is itself PHYSICAL, and so does the sudden-death drain. Both measured: a 10-point FIRE blast and a 4-point drain tick both landed in full on a fish with 8 armour.
- **Sandbag and Boxer together keep only the flat 3**, because `physical_reduction()` reads `magic_dmg()` — the arena's reckoning — and a boxer has already spent its magic on its fists. The same trade made twice, and it reads exactly as it should.
- It is **strong**: 21 rounds against a fish carrying no spells went 17–4 to the sandbagger. Fights do still finish — only one of the 21 needed the clock — but they run longer, and `flat` is the dial if absorbing a small fish's touch entirely turns out to be too much.
...
- **`_last_stand_used` is load-bearing, not bookkeeping.** When the clock runs out the fish kills itself through the ordinary damage path, so that blow arrives at `_refuse_killing_blow()` like any other — without the spent flag it would be refused in its turn and the fish would never die at all. Once per fight; `bind_fish()` hands it back with the health.
- The 10% is applied **last** in `phys_dmg()` / `magic_dmg()`, on whatever the other passives left: a boxer on its last legs swings a tenth of its converted total, not a tenth of what it started with.
- **It barely moves the needle, which is worth knowing before tuning anything else.** 21 rounds against a fish with no spells came out **11–10**, with 10 killing blows refused along the way — two seconds at a tenth of your power almost never turns a fight around. `power_scale` and `duration` are the dials if it should do more than buy a moment.
- A fish *can* win in those two seconds and then die anyway, since `BattleRound` calls the round the moment the other fish goes down; it did not happen once in those 21 rounds, but the arena would hand the win to a fish that is about to expire.
- **Decoy** (`data/spells/decoy.tres`, `CloneSpellData`) puts a copy of the fish on the floor beside it and points everything that was fighting it at the copy instead. The clone is a **real `BattleFish`** — same species, same icon, same size, and it flops about like anything else, which is the whole point — built from `fish.duplicate()` with `phys_dmg`, `magic_dmg` and `spells` emptied and `health_scale` (0.5) of the original's health. It is on the owner's team, so it never fights back and the owner never turns on it.
- **It is not one of the arena's battlers.** `BattleRound` was handed the real pair and never hears about the clone, so a decoy can neither win a round nor hold one open. Measured: while a clone was on the floor the arena still counted 2 battlers with 2 standing.
- **The lure is explicit.** `_lure_enemies()` walks the `battlers` group for anything that `can_hit()` the owner, remembers what each was chasing and repoints `chase_target` at the clone; `_end_clone()` hands those targets back, but only to fish still chasing the clone and only if the remembered target is itself still valid.
- **A coward is fooled by it too, and has to be told so separately.** A fighter follows `chase_target` and is lured by the fact of it being repointed; a coward decides for itself what to run from, so it would otherwise keep running from the nearest *real* fish. `is_decoy()` is what it asks — set once on the clone in `_spawn_clone()` — and a decoy beats proximity: measured with the clone moved to the far side of the coward, it ran 10.7 units the other way, straight past the real fish it was supposedly afraid of. With no decoy out it is unchanged: 1.6 → 21.7 units from the real one over 25 seconds. Measured end to end: enemy on the real fish → clone appears → enemy on the clone (PHYS 0, 0 spells) → clone gone at 3.89 s → enemy back on the real fish.
- **`is_instance_valid()` goes before the cast, never after.** `_lured` is the only place in the arena that keeps node references across time, and a fish freed while the clone is out is ordinary - a round torn down, a rematch. `pair[0] as BattleFish` on one of those is itself the error ("Trying to cast a freed object"), and it aborts the rest of `_end_clone()` with it, so the lures never get handed back. Every other `as BattleFish` in the codebase is on a live tree node or a fresh instance, which is why this is the one spot that needs the guard.
- **The clone keeps its own clock as well as the owner's.** `_tick_clone()` expires it, but a `SceneTreeTimer` set at spawn frees it too — because the owner's copy of the lifetime dies with the owner. The first draft had only the owner's clock, and an arena that frees a battler mid-round (a rematch does exactly that) left the decoy standing on the floor for the rest of the session with nothing alive that knew to take it away. Anything that outlives its caster needs its own lifetime; `SpellBubble` and `SpellFireZone` carry theirs for the same reason.
- **Fishnet** (`data/spells/fishnet.tres`, `WardSpellData`) is the magical half of what Sandbag does for a punch: `resistance` (0.3) of every **magical** hit never arrives. A share rather than a flat amount, which is the whole difference between the two — armour is worth most against a lot of small hits and can shrug one off entirely, a ward is worth the same proportion against a big spell as a small one and can never take a hit to nothing.
- Measured, on a fish with 30% ward: a 10-point FIRE blast cost it **7**; a 10-point touch cost it the full 10; a 10-point tick of **Wildfire** — a PHYSICAL spell — also cost the full 10, because a ward is no use against one of those; and a 10-point sudden-death tick landed in full. A warded fish also carrying Immunity was refused its killing blow at 1 health and then died on time when the stand ran out, which is the thing `UNTYPED` exists to protect.
- **Fish Lazer** (`data/spells/fish_lazer.tres`, `LaserSpellData`) plants the fish, winds up for `charge_time` (0.7 s) doing nothing at all, then runs a beam for `beam_time` (0.6 s), ticking every `interval` (0.15 s) for `damage_scale` (0.5) of its magic — four ticks, twice its magic, to anything that stands in the whole thing. It is LIGHTNING, so it is magical: `WardSpellData` turns a share of it aside and armour does nothing.
- **The aim is locked when the charge starts and never corrects**, which is the only answer to it. A fish that was in the line when the winding began and has flopped out of it by the time the light comes on has dodged the lot. It does **not** stop at the first fish it catches — measured, a target at 1.5 units and another at 4.5 both lost the full 20, while one 1.2 off the line and one behind the caster lost nothing.
- **It borrows the bubble volley's hang.** `_hang_left` is the arena's "this fish is held in place" timer, and a beam sets it the same way a volley does — which meant `_tick_volley()` had to learn to check `_volley != null` first, or it called off the beam's hang on its first frame. Whatever holds the fish, holds it; `_end_hang()` drops both.
- **The beam shoves nothing.** `take_blast()` is called with a knockback scale of 0, because a beam that knocked its target aside would blow it out of its own line after the first tick and the other three would hit empty floor.
- `SpellBeam` is a quad **standing in the world, not billboarded** — a beam's whole picture is which way it points, so it cannot turn to face the camera the way the fish do. The cost is the fish's own: a shot fired straight at or away from the camera is drawn nearly edge-on. The arena is 8 wide and 5 deep, so most shots run across it. The fish itself is washed towards `charge_tint` while it winds up, because a fish that stops dead for three quarters of a second and does not look like it meant to reads as a broken fish.
- **It is the most honestly balanced thing in the pool**, because standing still is expensive in an arena decided by touching: 15 rounds against a fish with no spells came out **8–7**, with 21 lasers fired, about 1.4 a round.
