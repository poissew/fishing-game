...
- Yaw and pitch are locked, roll is free: the fish faces the camera — **point the arena camera down −Z** — and tips end over end in the viewing plane when it flops. Pitch is locked because a capsule down the fish log-rolls about its own length far too easily, and a flat sprite doing that is edge-on half the time. `_face_the_tumble()` copies what is left onto the sprite and drops any yaw, since rotations about two axes compose into a turn about the third and a fish side-on to the camera is an invisible fish. The flop wiggle and the belly-up death pose ride on top as roll; `flip_h` faces it the way it is travelling.
