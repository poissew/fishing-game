...
- **Damage also says whether it is physical**, which is the only thing armour cares about. `take_damage()` is the touch path and is physical by definition; `take_blast()`, `take_tick_damage()` and `slam_down()` take a `physical` flag that defaults to false, and every spell passes `not data.is_magical()` so a PHYSICAL spell would be reduced and the rest are not. The arena's own sudden-death drain leaves it false on purpose: there would be no point in a clock that armour could sit out.
...
- **Sandbag** (`data/spells/sandbag.tres`, `ArmourSpellData`) is the third passive: every **physical** hit is worth `flat` (3) plus `per_power` (0.5) of the fish's magic damage less than it was thrown, to a minimum of nothing. A MAG 9 fish shrugs off 8, so a 10-damage touch costs it 2 and a 5-damage one costs it nothing at all.
