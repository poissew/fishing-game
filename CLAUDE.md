...
- **The lure is explicit.** `_lure_enemies()` walks the `battlers` group for anything that `can_hit()` the owner, remembers what each was chasing and repoints `chase_target` at the clone; `_end_clone()` hands those targets back, but only to fish still chasing the clone and only if the remembered target is itself still valid. Measured end to end: enemy on the real fish → clone appears → enemy on the clone (PHYS 0, 0 spells) → clone gone at 3.89 s → enemy back on the real fish.
