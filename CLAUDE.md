...
- **The sphere collider does the rotating.** Pitch and roll are free, so the fish tumbles the way it is thrown; only yaw is locked, so it keeps facing the camera — **point the arena camera down −Z.** `_face_the_tumble()` copies the body's pitch and roll onto the sprite each frame and drops the yaw: rolling about two axes at once composes into a turn about the third, and a fish that settles side-on to a flat sprite is an invisible fish. The flop wiggle and the belly-up death pose ride on top as roll, and `flip_h` faces it the way it is travelling.
